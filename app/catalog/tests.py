...
class CatalogTests(SimpleTestCase):
            ...
            with self.subTest(template=path.relative_to(template_dir)):
                ...
                for token in Lexer(path.read_text()).tokenize():
                    if token.token_type == TokenType.TEXT:
                        self.assertNotRegex(token.contents, r"\{[%{]", "Unrecognized Django template tag")
    ...
    def test_all_rendered_navigation_links_resolve(self):
        ...
        urls.extend(reverse("catalog:entry", args=[entry["id"], entry["slug"]])
            ...
            for link in parser.links:
                if link.startswith(("/", "#")):
                    with self.subTest(source=url, target=link):
                        ...
                        response = self.client.get(target.path + ("?" + target.query if target.query else ""))
                        self.assertEqual(response.status_code, 200)
                        if target.fragment:
    ...
    def test_named_cross_reference_and_return_link_target_specific_results(self):
        ...
        source_url = reverse("catalog:entry", args=[source["id"], source["slug"]])
        parser = LinkParser()
        ...
        self.assertEqual(response.context["return_url"], source_url + "#triple-sumset-lower-bound")
    ...
    def test_questions_are_collapsed_and_returning_to_an_answer_reopens_it(self):
        for entry in entries().values():
            ...
            parser = LinkParser()
            parser.feed(self.client.get(url).content.decode())
            questions = [item for item in parser.details if item.get("class") == "question-answer"]
            self.assertTrue(questions)
            self.assertTrue(all("open" not in question for question in questions))
        citation = next(anchor for anchor in parser.anchors
                        if urlsplit(anchor["href"]).fragment == "zero-summand")
        response = self.client.get(citation["href"])
        return_url = response.context["return_url"]
        self.assertEqual(urlsplit(return_url).fragment, "strict-growth-question")
        returned = self.client.get(return_url)
        parser = LinkParser()
        parser.feed(returned.content.decode())
        answers = [item for item in parser.details if item.get("class") == "question-answer"]
        self.assertEqual(sum("open" in item for item in answers), 1)

    def test_recorded_references_resolve_to_block_revisions(self):
        for entry in entries().values():
            for block in entry["blocks"]:
                for reference in block.get("references", []):
                    target = entries()[reference["entry_id"]]
                    self.assertIn(reference["block_id"], target["blocks_by_id"])
                    self.assertEqual(reference["revision"], target["revision"])

    def test_invalid_return_anchor_falls_back_to_the_source_article(self):
        source = entries()["thm-triple-sumset-lower-bound"]
        target = example_entry()
        target_url = reverse("catalog:entry", args=[target["id"], target["slug"]])
        for anchor in ["missing", "https://example.com/", "<script>"]:
            with self.subTest(anchor=anchor):
                response = self.client.get(target_url, {"from": source["id"], "at": anchor})
                self.assertEqual(response.context["return_url"],
                                 reverse("catalog:entry", args=[source["id"], source["slug"]]))
        ...
        for entry in entries().values():
            ...
            for source in [None, "missing", "https://example.com/", "//example.com/", entry["id"]]:
                with self.subTest(entry=entry["id"], source=source):
                    response = self.client.get(url, {} if source is None else {"from": source})
                    self.assertContains(response, "Back to Sumsets", count=2)
                    self.assertEqual(response.context["return_url"], area_link(areas()["sumsets"])["url"])
